...
const AddNewAdam = ({ open, onClose, fetchData }) => {
    ...
    const validate = () => {
        ...
        if (!formData.port.trim()) { newErrors.port = 'Port number is required'; }
        else if (!/^\d{1,}$/.test(formData.port.trim())) { newErrors.port = 'Port must be exactly three digits'; }
    ...
    const handleSubmit = async () => {
        if (!validate()) return;
        try {
            await mockApi.createAdam(formData);
            Notification.showSuccessMessage("Success", "Device added successfully");
            setFormData(initialValues);
            fetchData();
            onClose();
        } catch (error) {
            Notification.showErrorMessage("Error", "Failed to add device");
        }
    };
